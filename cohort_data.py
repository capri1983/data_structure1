...
def unique_houses(filename):
    ...
    # Code goes here
    houses_list = []
    with open('cohort_data.txt') as filename:
...
def hogwarts_by_house(filename):
    ...
    Iterate over the data to create an 
    - alphabeticaly sorted list for each house, and 
    - sorts students into their appropriate houses by last name. 
    - Sorts ghosts into a list called "ghosts" and 
    - instructors into a list called "instructors". 
    - Add them, in that order, to your list of houses.
...
def all_students_tuple_list(filename):
    ...
    student_list = []
    # student_list will be a list of tuples:
    # e.g. [ ('full name', 'house', 'advisor', 'cohort'). ('full name', 'house', 'advisor', 'cohort'), ('full name', 'house', 'advisor', 'cohort'), ('full name', 'house', 'advisor', 'cohort'), ('full name', 'house', 'advisor', 'cohort')  ]
    ...
    with open('cohort_data.txt') as filename:
        for line in filename:
            ...
            #     line_becomes_a_tuple = tuple(line)
            #     student_list.append(line_becomes_a_tuple)

            if line[-1] != 'I' and line[-1] != 'G':
                line = [line[0] + ' ' +line[1], line[2], line[3], line[4]]
                line_becomes_a_tuple = tuple(line)
                student_list.append(line_becomes_a_tuple)
